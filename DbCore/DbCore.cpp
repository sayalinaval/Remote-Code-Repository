...
bool testR3b()
{
  ...
  db["Arora"] = demoElem;
  Keys keys = db.keys();
  db["Fawcett"].children() = { "Salman","Sun","Prashar","Arora" };
  showHeader();
  showElem(db["Fawcett"]);
  db["Salman"].children() = { "Sun","Prashar","Arora" };
...
int main()
{
  ...
  ts1.test(testR1);
  ts1.testName("Use C++11");
  TestExecutive::TestStr ts2;
  ts2.test(testR2);
  ts2.testName("Use streams and new and delete");
  TestExecutive::TestStr ts3a;
  ts3a.test(testR3a);
  ts3a.testName("Create DbElement<std::string>");
  TestExecutive::TestStr ts3b;
  ts3b.test(testR3b);
  ts3b.testName("Create DbCore<std::string>");
  TestExecutive::TestStr ts4;
  ts4.test(testR4);
  ts4.testName("and and remove records");
  TestExecutive::TestStr ts5;
  ts5.test(testR5);
  ts5.testName("and and remove child relationships");
