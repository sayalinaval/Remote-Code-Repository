...
bool testR3b(DbCore<PayLoad>& db)
{
	...
	demoElem.payLoad().value("You should try ...");
	db["Salman"] = demoElem;
	demoElem.name("Jianan");
	demoElem.payLoad().value("Dr. Fawcett said ...");
	db["Sun"] = demoElem;
	demoElem.payLoad().value("You didn't demonstrate Requirement #4");
	demoElem.name("Nikhil");
	db["Prashar"] = demoElem;
	demoElem.payLoad().value("You didn't demonstrate Requirement #5");
