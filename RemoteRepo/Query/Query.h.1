...
namespace NoSqlDb
{
  ...
  public:
    ...
    Conditions() = default;
	void value(DbElement<P>& elem, std::string k1) { pDbElem_ = &elem, key_ = k1; } // set pointer to element used during query
  ...
  bool Conditions<P>::match()
  {
    try
    {
      return matchKey() && matchName() && matchDescription() && matchChildren() && matchDateTimeInterval();
    }
  ...
  Query<P>& Query<P>::select(Conditions<P>& conds)
  {
    ...
    for (auto item : db_)
    {
      conds.value(item.second, item.first);
      if (conds.match())
